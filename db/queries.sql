...
SELECT m.codigo AS codigo_curso,s.letra,m.nombre AS nombre_curso,m.numero_semestre,mc.carrera,s.cantidad_estudiantes FROM Seccion AS s INNER JOIN Materia AS m ON s.materia=m.codigo LEFT JOIN Materia_Carrera AS mc ON m.codigo=mc.materia ORDER BY carrera,numero_semestre;

-- Obtención de Secciones, consulta mejorada
SELECT m.codigo AS codigo_curso,s.letra,m.nombre AS nombre_curso,m.numero_semestre,mc.carrera,s.cantidad_estudiantes FROM Seccion AS s INNER JOIN Materia AS m ON s.materia=m.codigo LEFT JOIN Materia_Carrera AS mc ON m.codigo=mc.materia WHERE m.nombre='Matematica Basica 1' GROUP BY codigo_curso,letra ORDER BY carrera,numero_semestre;
SELECT m.codigo AS codigo_curso,s.letra,m.nombre AS nombre_curso,m.numero_semestre,mc.carrera,s.cantidad_estudiantes FROM Seccion AS s INNER JOIN Materia AS m ON s.materia=m.codigo LEFT JOIN Materia_Carrera AS mc ON m.codigo=mc.materia GROUP BY codigo_curso,letra ORDER BY carrera,numero_semestre;
